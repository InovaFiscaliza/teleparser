...
class VoLTEBenchmarkRunner:
    ...
    def create_synthetic_volte_data(self, size_kb: int) -> Path:
        ...
        basic_diameter_message = (
            (
                ...
                b"\x01"
                + length_bytes  # Version (1) + Length (3 bytes)
        ...
        for i in range(messages_needed):
            ...
            message = (
                basic_diameter_message[:-8]
                + (0x12345678 + i).to_bytes(4, "big")
                + b"\x87\x65\x43\x21"
            )
            test_data += message
    ...
    def run_comprehensive_benchmark(
                ...
                for decoder_name, decoder_factory in decoder_configs:
                    ...
                    try:
                        result = self.benchmark_decoder(
                            ...
                            decoder_name,
                            test_file,
                            show_progress=(size_kb == synthetic_sizes[0]),
                        )
        ...
        for test_file in test_files:
            ...
            for decoder_name, decoder_factory in decoder_configs:
                ...
                try:
                    result = self.benchmark_decoder(
                        decoder_factory, decoder_name, test_file
                    )
                    all_results.append(result)
    ...
    def print_comparison_table(self, results: List[BenchmarkResult]):
        ...
        for size_key, size_results in grouped.items():
            print(f"\n--- {size_key} Files ---")
            print(
                f"{'Decoder':<25} {'Rec/s':<10} {'MB/s':<8} {'Mem(MB)':<10} {'Speedup':<10}"
            )
            print("-" * 70)
            ...
            for result in size_results:
                speedup = (
                    result.records_per_second / original_rps if original_rps > 0 else 0
                )
                print(
                    f"{result.decoder_name:<25} {result.records_per_second:<10.0f} "
    ...
    def save_results(self, results: List[BenchmarkResult], output_file: str):
        ...
        with open(output_file, "w") as f:
            json.dump({"timestamp": time.time(), "results": results_dict}, f, indent=2)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark EricssonVolte decoder optimizations"
    )
    parser.add_argument(
        "--sizes",
        nargs="+",
    ...
    parser.add_argument(
        "--files", nargs="*", type=Path, default=[], help="Real test files to benchmark"
    )
    parser.add_argument(
        "--runs", type=int, default=5, help="Number of benchmark runs per configuration"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="volte_benchmark_results.json",
    ...
    if HAS_PARALLEL:
        decoder_configs.extend(
            [
                (
                    "Parallel (2 workers)",
                    lambda buffer: EricssonVolteParallel(buffer, n_workers=2),
                ),
                (
                    "Parallel (4 workers)",
                    lambda buffer: EricssonVolteParallel(buffer, n_workers=4),
                ),
            ]
        )
    ...
    if HAS_TWO_PHASE:
        decoder_configs.extend(
            [
                (
                    "Two-Phase (1 worker)",
                    lambda buffer: EricssonVolteTwoPhase(buffer, n_workers=1),
                ),
                (
                    "Two-Phase (2 workers)",
                    lambda buffer: EricssonVolteTwoPhase(buffer, n_workers=2),
                ),
                (
                    "Two-Phase (4 workers)",
                    lambda buffer: EricssonVolteTwoPhase(buffer, n_workers=4),
                ),
            ]
        )
